...
// ============================================
// Cada REST_INTERVAL_MINUTES (20 min por defecto), una sesión descansa.
...
//   ... repite
// ============================================
const REST_INTERVAL_MINUTES = parseInt(process.env.SESSION_REST_INTERVAL_MINUTES) || 20;
const REST_ENABLED = process.env.SESSION_REST_ENABLED !== 'false'; // Habilitado por defecto
const MIN_ACTIVE_BEFORE_REST_MS = (parseInt(process.env.SESSION_MIN_ACTIVE_BEFORE_REST_MINUTES) || 20) * 60 * 1000; // Mínimo 20 min activa antes de poder descansar
let restRotationStartTime = Date.now();

// Timestamps de cuándo cada sesión se puso READY (para no descansar sesiones recién conectadas)
const sessionReadyTimestamps = new Map();
...
/**
 * Registra el timestamp de cuando una sesión quedó READY
 * @param {string} sessionName
 */
function registerSessionReady(sessionName) {
    sessionReadyTimestamps.set(sessionName, Date.now());
    console.log(`⏱️ [DESCANSO] ${sessionName} registrada como activa. Mínimo ${MIN_ACTIVE_BEFORE_REST_MS / 60000} min antes de poder descansar.`);
}

/**
 * Limpia el timestamp de una sesión (al desconectarse)
 * @param {string} sessionName
 */
function clearSessionReady(sessionName) {
    sessionReadyTimestamps.delete(sessionName);
}
...
function getRestingSession() {
    ...
    // Verificar que la sesión lleva suficiente tiempo activa antes de ponerla a descansar
    const readyTimestamp = sessionReadyTimestamps.get(restingSession.name);
    const activeTime = readyTimestamp ? (Date.now() - readyTimestamp) : Infinity;
    
    if (activeTime < MIN_ACTIVE_BEFORE_REST_MS) {
        const minutesActive = Math.floor(activeTime / 60000);
        const minRequired = MIN_ACTIVE_BEFORE_REST_MS / 60000;
        console.log(`⏱️ [DESCANSO] ${restingSession.name} lleva solo ${minutesActive} min activa (mínimo ${minRequired} min). NO descansa aún.`);
        return null; // No poner a descansar, todas activas temporalmente
    }
...
function getRotationInfo() {
    ...
    // Calcular tiempo restante del periodo de gracia (cuando todas activas)
    let gracePeriodMsRemaining = 0;
    let nextRestingSession = null;
    if (!restInfo && REST_ENABLED && allReady.length >= 3) {
        const periodMs = REST_INTERVAL_MINUTES * 60 * 1000;
        const elapsed = Date.now() - restRotationStartTime;
        const periodStart = Math.floor(elapsed / periodMs) * periodMs;
        const periodEnd = periodStart + periodMs;
        gracePeriodMsRemaining = Math.max(0, periodEnd - elapsed);
        // La próxima sesión que descansará es la primera (índice 0 del ciclo)
        nextRestingSession = allReady[0]?.name || null;
    }
    ...
    return {
        ...
        rest: {
            ...
            gracePeriodMsRemaining: gracePeriodMsRemaining,
            gracePeriodMinutesRemaining: Math.ceil(gracePeriodMsRemaining / 60000),
            nextRestingSession: nextRestingSession,
