...
async function handleMenuSelection(socket, senderPhone, input, conversation) {
    ...
    if (option === '1') {
        ...
        await socket.sendMessage(senderPhone, {
            text: `🚗 *Asignar usuario a vehículo*\n\n` +
        ...
        await socket.sendMessage(senderPhone, {
            text: `📍 *Consultar placa y ubicación*\n\n` +
        ...
        await socket.sendMessage(senderPhone, {
            text: `❌ Opción no válida. Por favor, envía *1*, *2* o *3*.`
        });
    }
}
...
async function handleOption1Email(socket, senderPhone, input, conversation) {
    ...
    await socket.sendMessage(senderPhone, { text: `🔍 Validando correo: *${email}*...` });
    ...
    try {
        ...
        await socket.sendMessage(senderPhone, {
            text: `❌ Error al validar el correo: ${error.message}\n\nIntenta de nuevo o escribe *menu*.`
        });
        endConversation(senderPhone);
    }
}
...
async function handleOption1Plate(socket, senderPhone, input, conversation) {
    ...
    if (!isValidPlateFormat(formattedPlate)) {
        await socket.sendMessage(senderPhone, {
            text: `❌ Formato de placa inválido: *${plate}*\n\nEjemplo: IMU148 o IMU-148\n\nEnvía la placa nuevamente.`
        });
        return;
    }

    await socket.sendMessage(senderPhone, { text: `🔍 Validando placa: *${formattedPlate}*...` });
...
async function handleOption2Plate(socket, senderPhone, input, conversation) {
    ...
    await socket.sendMessage(senderPhone, { text: `🔍 Consultando placa: *${formattedPlate}*...` });
    ...
    try {
        ...
        await socket.sendMessage(senderPhone, {
            text: `❌ Error al consultar: ${error.message}\n\nEscribe *menu* para volver.`
        });
        endConversation(senderPhone);
    }
}
...
async function handleOption3Email(socket, senderPhone, input, conversation) {
    ...
    await socket.sendMessage(senderPhone, { text: `🔍 Consultando usuario: *${email}*...` });
    ...
    try {
        ...
        if (!user) {
            await socket.sendMessage(senderPhone, {
                text: `❌ No se encontró usuario con correo: *${email}*\n\nVerifica que esté registrado.\n\nEscribe *menu* para volver.`
            });
            endConversation(senderPhone);
            return;
        }
