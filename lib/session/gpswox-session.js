    ...
    try {
        ...
        await socket.sendMessage(senderPhone, {
            text: `✅ ¡Usuario encontrado!\n\n` +
                  ...
                  `🆔 ID: ${user.id}\n\n` +
                  `Ahora, por favor envía la *placa del vehículo* que deseas asignar.\n\n` +
                  `Formato: ABC123 o ABC-123\n` +
                  `(El sistema agregará el guion automáticamente si no lo incluyes)`
        });
...
async function handlePlateInput(session, socket, senderPhone, messageText, conversation) {
    ...
    try {
        ...
        const deviceInfo = device.user_id 
            ? `⚠️ Este vehículo ya está asignado al usuario ID: ${device.user_id}` 
        ...
        if (result.success) {
            // Obtener dispositivos actuales del usuario
            const userDevices = await getUserDevices(conversation.data.user.id);
            const deviceList = userDevices.map(d => `  • ${d.plate || d.name || d.imei}`).join('\n');

            await socket.sendMessage(senderPhone, {
                text: `✅ ¡Asignación exitosa!\n\n` +
                      `👤 Usuario: *${conversation.data.user.name || conversation.data.email}*\n` +
                      `🚗 Vehículo: *${formattedPlate}*\n\n` +
                      `📋 Vehículos asignados (${userDevices.length}):\n${deviceList || '  (ninguno)'}\n\n` +
                      `🎉 El proceso ha finalizado correctamente.\n\n` +
                      `Si deseas registrar otro vehículo, envía el correo electrónico del usuario.`
            });
