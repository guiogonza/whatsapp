...
async function showMenu(socket, senderPhone) {
    const menuText = `👋 *Bienvenido a la plataforma GPS*\n\n` +
...
async function handleMenuSelection(socket, senderPhone, input, conversation) {
    ...
    if (option === '1') {
        ...
        const responseText = `🔧 *Registrar vehículo no operativo*\n\n` +
                  ...
                  `*operatividad tulua*\n` +
                  `*operatividad zarzal*\n\n` +
                  `Luego te pediré la placa, la validaré en la plataforma GPS y solicitaré el estado y la observación.`;
        await database.logGPSwoxMessage(cleanPhone, 'OUT', responseText, 'MENU');
        await socket.sendMessage(senderPhone, { text: responseText });
    } else if (option === '5') {
        endConversation(senderPhone);
        const responseText = `📋 *Consultar vehículos no operativos*\n\n` +
                  `Envía la consulta con este formato:\n\n` +
                  `*consultar operatividad jamundi*\n` +
                  `*consultar operatividad buga*\n` +
                  `*consultar operatividad tulua*\n` +
                  `*consultar operatividad zarzal*\n` +
                  `*consultar operatividad todos*`;
        await database.logGPSwoxMessage(cleanPhone, 'OUT', responseText, 'MENU');
        await socket.sendMessage(senderPhone, { text: responseText });
    } else {
        const responseText = `❌ Opción no válida. Por favor, envía *1*, *2*, *3*, *4* o *5*.`;
        await database.logGPSwoxMessage(cleanPhone, 'OUT', responseText, 'MENU');
        await socket.sendMessage(senderPhone, { text: responseText });
    }
}
