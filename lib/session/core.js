...
async function createSession(sessionName) {
    ...
    try {
        ...
        // Capturar mapeos LID ↔ PN de los logs de Baileys
        const originalChildFn = logger.child.bind(logger);
        logger.child = function(bindings) {
            // Capturar mapeos LID cuando Baileys los reporta
            if (bindings && bindings.pnUser && bindings.lidUser) {
                registerLidMapping(bindings.lidUser, bindings.pnUser);
            }
            return originalChildFn(bindings);
        };
