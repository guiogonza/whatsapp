...
async function createSession(sessionName) {
    ...
    try {
        ...
        const socketConfig = {
            ...
            // Usar un browser fingerprint más realista y común
            browser: ['Chrome (Linux)', 'Chrome', '130.0.6723.91'],
            auth: {
                creds: state.creds,
                keys: makeCacheableSignalKeyStore(state.keys, logger)
            },
            generateHighQualityLinkPreview: true,
            // Sincronización completa para evitar errores de verificación
            syncFullHistory: false,
            // Marcar como conexión online para mejor sincronización
            markOnlineOnConnect: true,
            // Configuración de keep-alive para mantener conexión estable
            keepAliveIntervalMs: 30000,
            // Tiempo de espera para reintentos de conexión
            retryRequestDelayMs: 2000,
            // Tiempo máximo de espera para QR
            qrTimeout: 40000,
            // Mensaje de fallback cuando no se encuentra el mensaje original
            getMessage: async (key) => {
                // Retornar undefined para que Baileys maneje el caso correctamente
                return undefined;
            }
        };
...
async function handleConnectionClose(session, sessionName, lastDisconnect) {
    ...
    const isSyncError = statusCode === 440 || errorMessage.includes('sync') || errorMessage.includes('Stream Errored');

    // Manejar error de sincronización (440) - "Se cerró la sesión de tus dispositivos"
    if (isSyncError) {
