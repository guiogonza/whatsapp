...
async function handleConnectionClose(session, sessionName, lastDisconnect) {
    ...
    if (shouldReconnect) {
        ...
        if (session.isBeingDeleted) {
            console.log(`Sesión ${sessionName} eliminada intencionalmente, no se reconectará`);
...
async function handleIncomingMessage(session, sessionName, m, socket) {
    ...
    if (!message.key.fromMe && m.type === 'notify') {
        ...
        if (remoteJid.endsWith('@lid')) {
            // Es un LID, intentar obtener el número real
            const lidNumber = remoteJid.replace('@lid', '');
            
            // Buscar en el store de Baileys si existe
            if (socket.store && socket.store.contacts) {
                const contact = Object.values(socket.store.contacts).find(c => 
                    c.lid && c.lid.includes(lidNumber)
                );
                if (contact && contact.id) {
                    senderPhone = contact.id;
                    senderNumber = contact.id.replace('@s.whatsapp.net', '').replace('@c.us', '');
                    console.log(`🔗 LID ${lidNumber} resuelto a número: ${senderNumber}`);
                }
            }
            
            // Alternativa: usar message.verifiedBizName o extraer de participant si es grupo
