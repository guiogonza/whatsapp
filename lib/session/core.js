...
const preopFlow = require('./preop-flow');
const inspFlow = require('./inspeccion-flow');
...
// Sesiones que deben usar conexión directa (proxy falló múltiples veces)
const forceDirectConnectionSessions = new Set();
...
async function createSession(sessionName) {
    ...
    try {
        ...
        // Si la sesión falló múltiples veces con proxy, usar conexión directa
        let currentProxyAgent = null;
        if (forceDirectConnectionSessions.has(sessionName)) {
            console.log(`🌐 ${sessionName}: forzando conexión directa (proxy falló previamente)`);
            releaseProxyForSession(sessionName);
        } else {
            currentProxyAgent = await getProxyAgentForSession(sessionName);
        }
        if (currentProxyAgent) {
            console.log(`🌐 Proxy SOCKS5 asignado a sesión: ${sessionName} (IP única)`);
        } else if (!forceDirectConnectionSessions.has(sessionName)) {
        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                // Registrar timestamp de sesión READY para el sistema de descanso rotativo
                rotation.registerSessionReady(sessionName);
                ...
                forceDirectConnectionSessions.delete(sessionName);
...
async function handleConnectionClose(session, sessionName, lastDisconnect) {
    // Limpiar timestamp de sesión READY al desconectarse
    rotation.clearSessionReady(sessionName);
    ...
    const errorTrace = lastDisconnect?.error?.toString() || '';
    let shouldReconnect = statusCode !== DisconnectReason.loggedOut;
    const isLoggedOut = statusCode === DisconnectReason.loggedOut || statusCode === 401;
    const isForbidden = statusCode === 403; // Número bloqueado/baneado
    const isRestartRequired = statusCode === DisconnectReason.restartRequired || statusCode === 515;
    
    // Detectar error de proxy SOCKS5 (autenticación fallida)
    const isProxyAuthError = statusCode === 500 && 
        (errorMessage.includes('Socks5 Authentication failed') || 
         errorTrace.includes('Socks5 Authentication failed'));
    
    if (isProxyAuthError) {
        const proxyRetryCount = (session.proxyRetryCount || 0) + 1;
        console.log(`🌐❌ ${sessionName} error de autenticación SOCKS5 (intento ${proxyRetryCount}/3). Liberando proxy...`);
        releaseProxyForSession(sessionName);
        
        if (proxyRetryCount >= 3) {
            console.log(`⚠️ ${sessionName} falló con 3 proxies distintos. Conectando sin proxy...`);
            forceDirectConnectionSessions.add(sessionName);
        }
...
async function handleIncomingMessage(session, sessionName, m, socket) {
    ...
    if (!message.key.fromMe && m.type === 'notify') {
            ...
            if (fxSessionNames.length === 0) {
                ...
                if (!fxSocketFound) {
                    ...
                    const sendViaSesionFX = async (targetPhone, text) => {
                        return await sendMessageWithRetry(fxSocketFound, targetPhone, text, 3);
                    };
        ...
        // 💬 COMANDOS FX INTERACTIVOS (fx, posiciones, balance)
        const fxCommands = require('./fx-commands');
        if (fxCommands.isFXCommand(incomingText)) {
            console.log(`💬 ${sessionName} detectó comando FX de ${senderNumber}: "${incomingText}"`);
            try {
                const sendResponse = async (jid, content) => {
                    return await socket.sendMessage(jid, content);
                };
                const handled = await fxCommands.handleFXCommand(senderPhone, incomingText, sendResponse, message.key.remoteJid);
                if (handled) {
                    console.log(`✅ Comando FX procesado para ${senderNumber}`);
                    return; // No seguir con otras lógicas
                }
            } catch (cmdError) {
                console.error(`❌ Error procesando comando FX:`, cmdError.message);
            }
        }
        ...
        // 📋 BOT DE PREOPERACIONAL (Hesego) - reutiliza la sesión dedicada GPSwox
        // Dispara SOLO si el texto es un trigger explícito (PREOP/PREOPERACIONAL) o si el
        // remitente ya tiene una conversación de preoperacional en curso. Si ninguna de las
        // dos condiciones se cumple, no se toca nada y el flujo original (incl. GPSwox) sigue igual.
        if (message.message) {
            const preopIsTrigger = config.PREOP_TRIGGER_KEYWORDS.includes((incomingText || '').trim().toUpperCase());
            const preopHasSession = preopIsTrigger ? false : await preopFlow.hasActiveSession(senderPhone);
            if (preopIsTrigger || preopHasSession) {
                console.log(`📋 ${sessionName} detectó mensaje de Preoperacional de ${senderNumber}`);
                await preopFlow.handleMessage(session, sessionName, senderPhone, incomingText, message, socket);
                return;
            }
        }

        // 🔧 BOT DE INSPECCIÓN (Hesego) - mismo patrón que Preoperacional arriba,
        // módulo hermano independiente (checklist fijo de 20 ítems, reporte de daños).
        if (message.message) {
            const inspIsTrigger = config.INSP_TRIGGER_KEYWORDS.includes((incomingText || '').trim().toUpperCase());
            const inspHasSession = inspIsTrigger ? false : await inspFlow.hasActiveSession(senderPhone);
            if (inspIsTrigger || inspHasSession) {
                console.log(`🔧 ${sessionName} detectó mensaje de Inspección de ${senderNumber}`);
                await inspFlow.handleMessage(session, sessionName, senderPhone, incomingText, message, socket);
                return;
            }
        }
        ...
        if (message.message) {
            ...
            // Si es sesión GPSwox (dedicada), reenviar el mensaje al proyecto
            // hesego-operatividad (extraido de este monolito) via webhook.
            // Este servidor sigue siendo dueño de la conexión WhatsApp; solo
            // delega el procesamiento del flujo conversacional.
            if (isGPSwoxDedicatedSession) {
                if (config.GPSWOX_WEBHOOK_URL) {
                    console.log(`🔒 Sesión GPSwox dedicada - reenviando a hesego-operatividad`);
                    try {
                        ...
                        console.error(`❌ Error reenviando a hesego-operatividad: ${webhookError.message}`);
                    }
                    return; // No procesar otras respuestas automáticas
                }

                // Fallback: sin GPSWOX_WEBHOOK_URL configurado, procesar localmente
                // (comportamiento anterior, por si hesego-operatividad no está desplegado)
                console.log(`🔒 Sesión GPSwox dedicada - procesando flujo GPSwox localmente (puede iniciar nuevas conversaciones)`);
...
/**
 * Reconecta una sesión existente aplicando proxy
 * Útil para aplicar proxies a sesiones que fueron creadas sin ellos
 * NO elimina las credenciales, solo reconecta
 * @param {string} sessionName - Nombre de la sesión a reconectar
 * @returns {Promise<Object>} - La sesión reconectada
 */
async function reconnectSession(sessionName) {
    console.log(`🔄 Reconectando sesión ${sessionName} con proxy...`);
    
    // Verificar que la sesión existe
    const session = sessions[sessionName];
    if (!session) {
        throw new Error(`Sesión ${sessionName} no existe en memoria`);
    }
    
    // Verificar que existen las credenciales en disco
    const authPath = path.join(config.SESSION_DATA_PATH, sessionName);
    try {
        await fs.access(authPath);
    } catch (error) {
        throw new Error(`No se encontraron credenciales para ${sessionName}`);
    }
    
    // Cerrar la sesión sin hacer logout (mantiene credenciales)
    const tempDeleted = session.isBeingDeleted;
    session.isBeingDeleted = false; // Evitar que se marque como eliminada
    
    try {
        if (session.socket) {
            console.log(`📴 Cerrando socket de ${sessionName}...`);
            try {
                session.socket.end(undefined);
            } catch (endErr) {
                console.log(`⚠️ Error cerrando socket: ${endErr.message}`);
            }
        }
        
        // Liberar el proxy anterior si existe
        releaseProxyForSession(sessionName);
        ...
        // Recrear la sesión (esto asignará un nuevo proxy automáticamente)
        console.log(`🔄 Recreando sesión ${sessionName}...`);
        const newSession = await createSession(sessionName);
        
        console.log(`✅ Sesión ${sessionName} reconectada con éxito`);
        return newSession;
        
    } catch (error) {
        session.isBeingDeleted = tempDeleted;
        throw error;
    }
}

/**
 * Reconecta todas las sesiones activas aplicando proxies
 * Excluye sesiones GPSwox que no deben usar proxy
 * @param {boolean} excludeGpswox - Si es true, no reconecta sesiones GPSwox
 * @returns {Promise<Object>} - Resultado de las reconexiones
 */
async function reconnectAllSessions(excludeGpswox = true) {
    console.log(`🔄 Iniciando reconexión masiva de sesiones...`);
    
    const allSessions = Object.keys(sessions);
    const gpswoxSessions = config.GPSWOX_SESSION_NAMES || [];
    const fxSessions = config.FX_SESSION_NAMES || [];
    
    const toReconnect = allSessions.filter(name => {
        if (excludeGpswox && gpswoxSessions.includes(name)) {
            console.log(`⏭️ Saltando ${name} (sesión GPSwox - usa conexión directa)`);
            return false;
        }
        return true;
    });
    
    console.log(`📋 Sesiones a reconectar: ${toReconnect.length}/${allSessions.length}`);
    
    const results = {
        success: [],
        failed: [],
        skipped: allSessions.length - toReconnect.length
    };
    
    for (const sessionName of toReconnect) {
        try {
            console.log(`\n🔄 Reconectando ${sessionName}...`);
            await reconnectSession(sessionName);
            results.success.push(sessionName);
            console.log(`✅ ${sessionName} reconectada exitosamente`);
            
            // Esperar entre reconexiones para evitar sobrecargar
            await sleep(3000);
        } catch (error) {
            console.error(`❌ Error reconectando ${sessionName}:`, error.message);
            results.failed.push({ name: sessionName, error: error.message });
        }
    }
...
function getSessionsStatus() {
    ...
    return Object.entries(sessions).map(([name, session]) => {
        ...
        return {
            ...
            socket: !!session.socket, // Indica si el socket está activo
