...
async function handleIncomingMessage(session, sessionName, m, socket) {
    ...
    if (!message.key.fromMe && m.type === 'notify') {
        ...
        if (message.message) {
            ...
            const dedicatedModeEnabled = isGPSwoxDedicatedMode();
            
            // Si es sesión dedicada GPSwox, SOLO procesar GPSwox (ignorar todo lo demás)
            if (isGPSwoxDedicatedSession && dedicatedModeEnabled) {
                console.log(`🔒 Sesión GPSwox dedicada - solo procesando flujo GPSwox`);
                const gpswoxProcessed = await processGPSwoxMessage(session, sessionName, socket, senderPhone, incomingText);
                
                if (!gpswoxProcessed && !hasActiveConversation(senderPhone)) {
                    // Enviar mensaje de bienvenida si no hay conversación activa
                    // Usar senderPhone (número resuelto) en vez de remoteJid (puede ser LID)
                    await socket.sendMessage(senderPhone, {
                        text: `👋 ¡Bienvenido al sistema de registro vehiculo!\n\n` +
                              `Para comenzar, por favor envía tu *correo electrónico* registrado en el sistema.\n\n` +
            ...
            // PRIORIDAD 1: Procesar flujo GPSwox si hay conversación activa o si el mensaje parece iniciar una
            const isGPSwoxConversation = hasActiveConversation(senderPhone);
            const gpswoxProcessed = await processGPSwoxMessage(session, sessionName, socket, senderPhone, incomingText);
