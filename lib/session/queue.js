...
async function processConsolidationQueue() {
    ...
    // Construir RONDAS: cada ronda tiene 1 mensaje por canal (sesión/cloud), cada uno a un NÚMERO DIFERENTE
    // Ej: 3 sesiones → Ronda 1: [S1→NumA, S2→NumB, S3→NumC], Ronda 2: [S1→NumD, S2→NumE, S3→NumF]
    const rounds = [];
    let numberIdx = 0;
    const channelsPerRound = (cloudApiReady ? 1 : 0) + activeSessions.length;
    
    while (numberIdx < numbersData.length) {
        const round = [];
        
        // 1 mensaje para Cloud API si está disponible y hay números pendientes
        if (cloudApiReady && numberIdx < numbersData.length) {
            round.push({
                type: 'cloud',
    ...
    console.log(`[COLA] ${totalTasks} tareas en ${rounds.length} rondas (${channelsPerRound} canales por ronda: ${activeSessions.length} sesiones${cloudApiReady ? ' + 1 Cloud API' : ''})`)
    console.log(`[COLA] Cada ronda: 1 mensaje consolidado por sesión, cada uno a un número diferente\n`);
