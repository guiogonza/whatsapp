...
async function processConsolidationQueue() {
    ...
    // Calcular distribución 50/50
    const totalMessages = numbersData.length;
    const cloudPercentage = hybridEnabled ? (config.WHATSAPP_CLOUD_PERCENTAGE || 50) : 0;
    const cloudCount = hybridEnabled ? Math.floor(totalMessages * (cloudPercentage / 100)) : 0;
    const baileysCount = totalMessages - cloudCount;
    ...
    if (hybridEnabled) {
        ...
        console.log(`[HÍBRIDO] Distribución: ${cloudCount} Cloud API (${cloudPercentage}%) | ${baileysCount} Baileys (${100 - cloudPercentage}%)`);
        console.log(`[HÍBRIDO] Cloud API: ${apiStats.hourlyCount}/${apiStats.hourlyLimit} msgs/hora | Baileys: ${activeSessions.length} sesiones activas`);
        console.log(`[HÍBRIDO] Sesiones: ${activeSessions.map(s => s.name).join(', ')}`);
    } else {
        console.log(`[BAILEYS] Usando ${activeSessions.length} sesiones: ${activeSessions.map(s => s.name).join(', ')}`);
    }
