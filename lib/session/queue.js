        ...
        for (const task of round) {
            ...
            if (task.type === 'cloud') {
                ...
                sendPromises.push((async () => {
                    try {
                        const sendResult = await cloudApi.sendMessage(phoneNumber, combinedMessage);
                        ...
                        if (sendResult.success) {
                            ...
                            console.error(`  ❌ Cloud API: Error → ${phoneNumber}: ${sendResult.error?.message || 'Error desconocido'}`);
                            return { success: false, method: 'cloud-api', phone: phoneNumber, error: sendResult.error };
                        }
                    } catch (error) {
