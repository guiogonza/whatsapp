...
async function processConsolidationQueue() {
    ...
    const hybridEnabled = config.HYBRID_MODE_ENABLED && cloudApiAvailable && cloudApiAccountReady;
    ...
    if (activeSessions.length === 0 && (!cloudApiAvailable || !cloudApiAccountReady)) {
    ...
    // Cloud API solo se usa si está disponible Y la cuenta está lista
    const cloudApiReady = cloudApiAvailable && cloudApiAccountReady;
