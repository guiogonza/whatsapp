...
async function processConsolidationQueue() {
    ...
    // Actualizar cache de conversaciones mensuales para verificar límite
    ...
    const hybridEnabled = config.HYBRID_MODE_ENABLED && cloudApiAvailable && cloudApiAccountReady && !cloudApiLimitReached;
    ...
    // Log de descanso rotativo (si hay alguna sesión descansando)
    if (_getRestingSession) {
        const restInfo = _getRestingSession();
        if (restInfo) {
            console.log(`😴 [DESCANSO] ${restInfo.sessionName} descansando (${restInfo.minutesRemaining} min restantes) | Sesiones enviando: ${activeSessions.map(s => s.name).join(', ')}`);
        }
    }
    
    // Si no hay ningún método de envío disponible, dejar en cola
    if (activeSessions.length === 0 && (!cloudApiAvailable || !cloudApiAccountReady || cloudApiLimitReached)) {
        const apiStats = cloudApi.getStats();
        
        if (!cloudApiAccountReady) {
            console.log(`[COLA] ⏸️ Cloud API no está lista (${apiStats.accountError || 'cuenta no registrada'})`);
        }
        
        if (allActiveSessions.length > 0) {
            console.log(`[COLA] ⏸️ Sesiones Baileys han alcanzado límite de ${config.MAX_MESSAGES_PER_SESSION_PER_HOUR} msgs/hora`);
        } else {
            console.log(`[COLA] ⏸️ No hay sesiones Baileys activas`);
        }
        
        console.log(`[COLA] 📬 ${numbersData.length} mensajes esperando en cola hasta que algún canal esté disponible`);
        return;
    }
    ...
    // Cloud API solo se usa si está disponible Y la cuenta está lista Y no superó el límite
    const cloudApiReady = cloudApiAvailable && cloudApiAccountReady && !cloudApiLimitReached;
    
    if (cloudApiReady && activeSessions.length > 0) {
        // Cloud API es PRIORIDAD: ratio 3:1 (3 Cloud API por cada 1 Baileys por sesión)
        const cloudPerRound = 3;
        const baileysPerRound = activeSessions.length; // 1 por sesión
        const totalPerRound = cloudPerRound + baileysPerRound;
        cloudPercentage = Math.round((cloudPerRound / totalPerRound) * 100);
        cloudCount = Math.floor(totalMessages * (cloudPerRound / totalPerRound));
        baileysCount = totalMessages - cloudCount;
    } else if (cloudApiReady && activeSessions.length === 0) {
    ...
    // Construir RONDAS: Cloud API es prioridad (3 msgs), luego 1 msg por sesión Baileys
    const rounds = [];
    let numberIdx = 0;
    const cloudMsgsPerRound = 3; // Cloud API envía 3 por ronda
    const channelsPerRound = (cloudApiReady ? cloudMsgsPerRound : 0) + activeSessions.length;
    
    while (numberIdx < numbersData.length) {
        const round = [];
        
        // 3 mensajes para Cloud API (prioridad)
        for (let c = 0; c < cloudMsgsPerRound && cloudApiReady && numberIdx < numbersData.length; c++) {
            round.push({
                type: 'cloud',
                data: numbersData[numberIdx]
            });
            numberIdx++;
        }
        
        // 1 mensaje por cada sesión Baileys activa (cada una a un número diferente)
        for (let s = 0; s < activeSessions.length && numberIdx < numbersData.length; s++) {
            round.push({
                type: 'baileys',
                data: numbersData[numberIdx],
                session: activeSessions[s]
            });
            numberIdx++;
        }
    ...
    const totalTasks = rounds.reduce((sum, r) => sum + r.length, 0);
    console.log(`[COLA] ${totalTasks} tareas en ${rounds.length} rondas (${channelsPerRound} canales por ronda: ${activeSessions.length} sesiones${cloudApiReady ? ' + 3 Cloud API' : ''})`)
    console.log(`[COLA] Cada ronda: 3 msgs Cloud API (prioridad) + 1 msg por sesión Baileys\n`);
    
    // Procesar RONDAS: en cada ronda todas las sesiones envían en paralelo (1 msg c/u a diferente número)
    // Luego esperar delay (30-60s) antes de la siguiente ronda
    let totalSuccess = 0;
    let totalFailed = 0;
    let totalCloudSuccess = 0;
    let totalBaileysSuccess = 0;
    
    for (let r = 0; r < rounds.length; r++) {
        const round = rounds[r];
        const sendPromises = [];
        
        console.log(`[RONDA ${r + 1}/${rounds.length}] Enviando ${round.length} mensajes en paralelo (1 por sesión, cada uno a diferente número)...`);
        ...
        for (const task of round) {
            ...
            if (task.type === 'cloud') {
                ...
                sendPromises.push((async () => {
                    try {
                        // Cloud API envía cada alerta como template individual (no consolidado)
                        let allSuccess = true;
                        let lastError = null;
                        
                        for (const msg of messages) {
                            const alertData = cloudApi.parseAlertMessage(msg.message);
                            let sendResult;
                            
                            if (alertData) {
                                // Es alerta GPS → usar template
                                sendResult = await cloudApi.sendAlertTemplate(phoneNumber, alertData);
                            } else {
                                // No es alerta → enviar como texto
                                sendResult = await cloudApi.sendTextMessage(phoneNumber, msg.message);
                            }
                        ...
                        if (allSuccess) {
                            ...
                            console.log(`  ✅ Cloud API: ${msgCount} templates → ${phoneNumber} ✓`);
                            ...
                            console.error(`  ❌ Cloud API: Error → ${phoneNumber}: ${lastError?.message || 'Error desconocido'}`);
                            return { success: false, method: 'cloud-api', phone: phoneNumber, error: lastError };
                        }
                    } catch (error) {
                ...
                sendPromises.push((async () => {
                    try {
                        const formattedNumber = formatPhoneNumber(phoneNumber);
                        const sendResult = await _sendMessageWithRetry(session, formattedNumber, combinedMessage, 3);
                        ...
                        if (sendResult.success) {
                            ...
                            console.error(`  ❌ ${session.name}: Error → ${phoneNumber}: ${sendResult.error?.message}`);
                            return { success: false, method: 'baileys', session: session.name, phone: phoneNumber, error: sendResult.error };
                        }
                    } catch (error) {
        ...
        if (sendPromises.length > 0) {
            ...
            const cloudOk = results.filter(r => r.success && r.method === 'cloud-api').length;
            const baileysOk = results.filter(r => r.success && r.method === 'baileys').length;
            
            totalSuccess += successful;
            totalFailed += failed;
            totalCloudSuccess += cloudOk;
            totalBaileysSuccess += baileysOk;
            
            console.log(`[RONDA ${r + 1}/${rounds.length}] ✓ ${successful} exitosos (☁️${cloudOk} 📱${baileysOk}) | ✗ ${failed} fallidos`);
        }
        
        // Esperar delay ENTRE RONDAS (cada sesión ya envió su mensaje consolidado a un número diferente)
    ...
    console.log(`\n[RESUMEN] ✓ ${totalSuccess} exitosos (☁️${totalCloudSuccess} 📱${totalBaileysSuccess}) | ✗ ${totalFailed} fallidos`);
    
    console.log(`\n${'='.repeat(60)}`);
    console.log(`[CONSOLIDACION] ✓ Procesamiento completado (${rounds.length} rondas)`);
    console.log(`${'='.repeat(60)}\n`);
}
