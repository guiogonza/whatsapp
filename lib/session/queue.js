...
const INVALID_TEMPLATE_ERROR = 'Mensaje descartado: no coincide con la plantilla alerta_vehiculo';
...
async function addToConsolidation(phoneNumber, message) {
    ...
    if (!alertData) {
        console.log(`[COLA] 🗑️ Mensaje descartado para ${formattedNumber}: formato de plantilla inválido`);
        logMessageSent('consolidation', formattedNumber, message, 'discarded', INVALID_TEMPLATE_ERROR);
        return {
            success: false,
            discarded: true,
    ...
    if (result.success) {
        ...
        try {
            ...
            if (cloudApiAvailable && !cloudApiLimitReached && !isQuietHours()) {
                const sendResult = await cloudApi.sendAlertTemplate(formattedNumber, alertData);
...
async function processConsolidationQueue() {
    ...
    for (let r = 0; r < rounds.length; r++) {
        ...
        const sendPromises = round.map((msg, idx) => (async () => {
            ...
            const formattedAlertMessage = alertData ? cloudApi.formatAlertMessage(alertData) : messageText;

            if (!normalizedPhone) {
                const invalidPhoneError = 'Número de teléfono inválido';
                await database.markMessagesSent([messageId], 'discarded-invalid-phone');
                logMessageSent('consolidation', phoneNumber, messageText, 'discarded', invalidPhoneError);
                console.log(`  🗑️ DESCARTADO → ${phoneNumber}: ${invalidPhoneError}`);
                return { success: false, discarded: true };
            }

            if (!alertData) {
                await database.markMessagesSent([messageId], 'discarded');
                logMessageSent('consolidation', phoneNumber, messageText, 'discarded', INVALID_TEMPLATE_ERROR);
                console.log(`  🗑️ DESCARTADO → ${phoneNumber}: plantilla inválida`);
                return { success: false, discarded: true };
            }
            
            if (cloudApiReady) {
                // Enviar por Cloud API como template individual
                const sendResult = await cloudApi.sendAlertTemplate(normalizedPhone, alertData);
                
                if (sendResult.success) {
                    await database.markMessagesSent([messageId]);
                    console.log(`  ✅ ☁️ → ${normalizedPhone} ✓`);
                    logMessageSent('cloud-api', normalizedPhone, formattedAlertMessage, 'sent', null, false, 1);
                    return { success: true };
                } else {
                    const errorMessage = sendResult.error?.message || 'Error';
                    await database.recordFailedAttempt(messageId, errorMessage);
                    console.error(`  ❌ ☁️ → ${normalizedPhone}: ${errorMessage}`);
                ...
                try {
                    const sendResult = await _sendMessageWithRetry(session, normalizedPhone, formattedAlertMessage, 3);
                    
                    if (sendResult.success) {
                        await database.markMessagesSent([messageId]);
                        trackMessageSent(session);
                        console.log(`  ✅ 📱 ${session.name} → ${normalizedPhone} ✓`);
                        logMessageSent(session.name, normalizedPhone, formattedAlertMessage, 'sent', null, false, 1);
                        return { success: true };
                    } else {
                        const errorMessage = sendResult.error?.message || 'Error enviando por Baileys';
                        await database.recordFailedAttempt(messageId, errorMessage);
                        console.error(`  ❌ 📱 ${session.name} → ${normalizedPhone}: ${errorMessage}`);
                        return { success: false };
                    }
                } catch (error) {
                    await database.recordFailedAttempt(messageId, error.message);
                    console.error(`  ❌ 📱 ${session.name} → ${normalizedPhone}: ${error.message}`);
                    return { success: false };
                }
            }
        })());
