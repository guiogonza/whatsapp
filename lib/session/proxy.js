...
/**
 * Obtiene la IP del proxy asignado a una sesión
 * @param {string} sessionName 
 * @returns {Promise<{ip: string|null, proxyUrl: string|null, location: string}>}
 */
async function getSessionProxyIP(sessionName) {
    const proxyUrl = sessionProxyMap.get(sessionName);
    if (!proxyUrl) {
        return { ip: null, proxyUrl: null, location: 'VPS Directo' };
    }
    ...
    if (global.proxyIPCache.has(proxyUrl)) {
        ...
        if (Date.now() - cached.timestamp < 5 * 60 * 1000) { // Cache 5 minutos
            return { ip: cached.ip, proxyUrl: maskProxy(proxyUrl), location: 'Proxy' };
        }
    }
    
    const ip = await getProxyPublicIP(proxyUrl);
    if (ip) {
        global.proxyIPCache.set(proxyUrl, { ip, timestamp: Date.now() });
    }
    
    return { ip, proxyUrl: maskProxy(proxyUrl), location: 'Proxy' };
}
