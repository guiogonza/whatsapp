...
// La plantilla aprobada de Meta (alerta_vehiculo) no tiene una variable propia
// para la velocidad, asi que se agrega como sufijo de "evento" en vez de
// crear una linea nueva (eso requeriria editar la plantilla y esperar
// re-aprobacion de Meta).
const VELOCIDAD_REGEX = /velocidad:?\s*\(?\s*([\d.,]+\s*(?:kph|km\/h|mph))\)?/i;

function appendVelocidad(evento, rawMessage) {
    const match = (rawMessage || '').match(VELOCIDAD_REGEX);
    if (!match) return evento;
    return `${evento} · ${match[1].trim()}`;
}
...
/**
 * Envía mensaje interactivo (botones o lista) usando WhatsApp Cloud API.
 * Solo puede usarse dentro de la ventana de servicio de 24h (el usuario ya escribió antes),
 * o el negocio recibirá un error 131047. Para abrir conversación usar sendTemplateMessage.
 *
 * interactive: objeto "interactive" tal como lo espera Meta, ej:
 *   { type: 'button', body: { text }, action: { buttons: [...] } }
 *   { type: 'list', header, body, footer, action: { button, sections } }
 */
async function sendInteractiveMessage(phoneNumber, interactive) {
    ...
    try {
        ...
        if (response.data?.messages?.[0]) {
            ...
            console.log(`✅ [CLOUD API] Interactivo (${interactive.type}) enviado a ${formattedPhone}`);

            return { success: true, messageId: response.data.messages[0].id, via: 'cloud-api-interactive' };
        }
        ...
        const errorMessage = metaError ? `${metaError.code}: ${metaError.message}` : error.message;

        if (metaError && ACCOUNT_NOT_READY_ERRORS.includes(metaError.code)) {
            apiStats.accountReady = false;
            apiStats.accountError = errorMessage;
            apiStats.lastAccountCheck = new Date();
        }

        console.error(`❌ [CLOUD API] Error enviando interactivo a ${formattedPhone}: ${errorMessage}`);
        return { success: false, error: new Error(errorMessage), via: 'cloud-api-interactive' };
    }
}
...
function parseAlertMessage(message) {
    ...
    if (gpswoxStarsMatch) {
        ...
        const ubicMatch = flattenedMessage.match(/(?:Ubicaci[oó]n|[Ll]ocation|[Dd]irecci[oó]n)[:\s]+(.+?)(?:\s+(?:Fecha|Hora|Time)[:\s]|\s*$)/i)
        ...
        return {
            ...
            evento: appendVelocidad(normalizeAlertValue(gpswoxStarsMatch[3], 'Alerta'), flattenedMessage),
    ...
    if (gpswoxPlainMatch) {
        ...
        return {
            ...
            evento: appendVelocidad(normalizeAlertValue(gpswoxPlainMatch[3], 'Alerta'), flattenedMessage),
            ubicacion: normalizeAlertValue(ubicMatch ? ubicMatch[1] : null, 'Ver GPS'),
            hora: normalizeAlertValue(horaMatch ? horaMatch[1] : null, 'Ver app')
        };
    }
