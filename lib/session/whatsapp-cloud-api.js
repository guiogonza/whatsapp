...
async function sendTemplateMessage(phoneNumber, templateName, languageCode = 'es', components = []) {
    ...
    try {
        console.log(`📤 [CLOUD API] Enviando template '${templateName}' (${languageCode}) a ${formattedPhone} con ${components[0]?.parameters?.length || 0} params`);
        const response = await axios.post(url, payload, {
            headers: {
                'Authorization': `Bearer ${config.WHATSAPP_CLOUD_TOKEN}`,
                'Content-Type': 'application/json'
...
function parseAlertMessage(message) {
    ...
    // Si la empresa viene del formato 📍Rastreamos, usar "Rastreamos"
    if (result.empresa) {
