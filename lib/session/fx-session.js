...
function subscribe(phoneNumber, accountNumber, notificationTypes = null) {
    ...
    saveSubscriberToDB(cleanPhone, accountStr, prefs.types);
...
/**
 * Guarda un suscriptor en BD para persistencia
 */
async function saveSubscriberToDB(phone, account, types) {
    try {
        await database.query(`
            INSERT INTO fx_subscribers (phone_number, account_number, notification_types, created_at)
            VALUES ($1, $2, $3, NOW())
            ON CONFLICT (phone_number, account_number) DO UPDATE SET
                notification_types = EXCLUDED.notification_types,
                updated_at = NOW()
        `, [phone, account, JSON.stringify(types)]);
    } catch (error) {
        ...
        if (error.code === '42P01') {
            try {
                ...
                console.error('❌ Error creando tabla fx_subscribers:', e.message);
            }
        } else {
...
/**
 * Carga suscriptores desde BD al iniciar
 */
async function loadSubscribersFromDB() {
    try {
        const result = await database.query(
            `SELECT phone_number, account_number, notification_types FROM fx_subscribers WHERE active = TRUE`
        );
        for (const row of result.rows) {
            const types = typeof row.notification_types === 'string' 
                ? JSON.parse(row.notification_types) 
                : (row.notification_types || Object.values(NOTIFICATION_TYPES));
            // Cargar en memoria sin volver a guardar en BD
            _loadSubscriberToMemory(row.phone_number, row.account_number, types);
        }
        console.log(`📊 ${result.rows.length} suscriptores FX cargados desde BD`);
    } catch (error) {
        if (error.code !== '42P01') {
...
/**
 * Carga un suscriptor en memoria (sin BD)
 */
function _loadSubscriberToMemory(phone, account, types) {
    if (!subscribers.has(account)) {
        subscribers.set(account, []);
    }
    if (!subscribers.get(account).includes(phone)) {
        subscribers.get(account).push(phone);
    }
    if (!userPreferences.has(phone)) {
        userPreferences.set(phone, { accounts: [], types: types });
    }
    const prefs = userPreferences.get(phone);
    if (!prefs.accounts.includes(account)) {
        prefs.accounts.push(account);
    }
}

// Cargar suscriptores desde BD con reintentos (la BD puede no estar lista al iniciar)
let loadRetries = 0;
const MAX_LOAD_RETRIES = 10;
const LOAD_RETRY_DELAY = 5000;

function tryLoadSubscribers() {
    loadSubscribersFromDB()
        .then(() => {
            if (loadRetries > 0) {
                console.log(`📊 Suscriptores FX cargados tras ${loadRetries} reintentos`);
            }
        })
        .catch(e => {
            loadRetries++;
            if (loadRetries <= MAX_LOAD_RETRIES) {
                console.log(`⏳ Reintentando carga de suscriptores FX (${loadRetries}/${MAX_LOAD_RETRIES})...`);
                setTimeout(tryLoadSubscribers, LOAD_RETRY_DELAY);
            } else {
                console.error('❌ No se pudieron cargar suscriptores FX después de', MAX_LOAD_RETRIES, 'intentos');
            }
        });
}

setTimeout(tryLoadSubscribers, 3000); // Esperar 3s a que la BD esté lista
