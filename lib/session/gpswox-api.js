...
async function findDeviceByPlate(plate) {
    try {
        ...
        // Buscar el dispositivo cuyo name coincida con la placa
        for (const group of groups) {
            const items = group.items || [];
            for (const device of items) {
                const deviceName = (device.name || '').trim().toUpperCase();
                if (deviceName === normalizedPlate) {
...
async function getDeviceUserIds(deviceId) {
    try {
        ...
        if (data && Array.isArray(data.users)) {
            ...
            console.log(`✅ Dispositivo ${deviceId} tiene ${userIds.length} usuarios asignados`);
            return userIds;
        }

        console.log(`⚠️ No se encontró campo users en edit_device_data`);
        return [];
...
async function assignDeviceToUser(userId, deviceId) {
    try {
        ...
        if (currentUserIds.includes(userId)) {
            console.log(`ℹ️ Usuario ${userId} ya está asignado al dispositivo ${deviceId}`);
        ...
        console.log(`📝 Enviando ${updatedUserIds.length} usuarios (${currentUserIds.length} existentes + 1 nuevo)`);
