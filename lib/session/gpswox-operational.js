...
const DOCUMENT_PROMPT_HOUR = 9;
const activeConversations = new Map();
let schedulerInterval = null;
let lastSchedulerRunKey = null;
let lastSchedulerDocRunKey = null;
...
function normalizeDocumentPayload(data = {}) {
    ...
    return {
        plate,
        siteName: String(data.siteName || data.site_name || '').trim() || null,
        documentType,
        expiryDate,
...
async function listDocumentExpirations(filters = {}) {
    ...
    if (filters.search) {
        params.push(`%${String(filters.search).trim()}%`);
        where.push(`(plate ILIKE $${params.length} OR document_type ILIKE $${params.length} OR site_name ILIKE $${params.length})`);
    }

    const selectSql = `
        SELECT id, plate, site_name, document_type, expiry_date, last_change_date, last_change_km, next_change_km,
               observation, active, created_by, created_at, updated_at,
               CASE WHEN expiry_date IS NULL THEN NULL ELSE to_char(expiry_date, 'DD/MM/YYYY') END AS expiry_date_co,
               CASE WHEN last_change_date IS NULL THEN NULL ELSE to_char(last_change_date, 'DD/MM/YYYY') END AS last_change_date_co,
               CASE WHEN expiry_date IS NULL THEN NULL ELSE (expiry_date - CURRENT_DATE)::int END AS days_remaining
    ...
    if (filters.paginate) {
        ...
        const result = await database.query(`
            ${selectSql}
            ORDER BY expiry_date ASC NULLS LAST, plate ASC, document_type ASC
            LIMIT $${queryParams.length - 1} OFFSET $${queryParams.length}
        `, queryParams);
        const total = countResult.rows[0]?.total || 0;
        return { rows: result.rows, pagination: buildPagination(total, page, limit) };
...
async function upsertDocumentExpiration(data = {}) {
    ...
    if (!payload.expiryDate && payload.documentType !== 'cambio_aceite') {
        throw new Error('Fecha de vencimiento invalida. Usa DD/MM/AAAA');
    }

    const result = await database.query(`
        INSERT INTO operational_document_expirations (
            plate, site_name, document_type, expiry_date, last_change_date, last_change_km, next_change_km, observation, created_by, active
        )
...
async function updateDocumentExpiration(id, data = {}) {
    ...
    `, [
        ...
        Object.prototype.hasOwnProperty.call(data, 'siteName') || Object.prototype.hasOwnProperty.call(data, 'site_name')
            ? (String(data.siteName || data.site_name || '').trim() || null)
            : current.site_name,
        documentType,
        expiryDate,
...
function buildDocumentFollowupMessage(siteName, docs) {
    const expired = docs.filter(d => d.days_remaining !== null && d.days_remaining < 0);
    const expiringSoon = docs.filter(d => d.days_remaining !== null && d.days_remaining >= 0 && d.days_remaining <= 30);
    const noDate = docs.filter(d => d.days_remaining === null);

    const lines = [
        `📋 *Seguimiento documentos - 9 a.m.*`,
        `📍 Sede: *${siteName}*`,
        ''
    ];

    if (expired.length > 0) {
        lines.push(`❌ *Documentos vencidos (${expired.length}):*`);
        expired.forEach(d => {
            lines.push(`• ${d.plate} - ${getDocumentTypeLabel(d.document_type)} (vencio ${d.expiry_date_co || '-'})`);
        });
        lines.push('');
    }

    if (expiringSoon.length > 0) {
        lines.push(`⚠️ *Vencen en los proximos 30 dias (${expiringSoon.length}):*`);
        expiringSoon.forEach(d => {
            lines.push(`• ${d.plate} - ${getDocumentTypeLabel(d.document_type)} (${d.days_remaining} dias, ${d.expiry_date_co || '-'})`);
        });
        lines.push('');
    }

    if (noDate.length > 0) {
        lines.push(`🔧 *Cambios aceite sin fecha proximo cambio (${noDate.length}):*`);
        noDate.forEach(d => {
            lines.push(`• ${d.plate} - ${getDocumentTypeLabel(d.document_type)}`);
        });
        lines.push('');
    }

    lines.push('📝 Para actualizar, escribe: *vencimientos*');
    return lines.join('\n');
}

async function sendDocumentFollowups(sessionManager, options = {}) {
    const sendType = options.sendType === 'manual' ? 'manual' : 'automatico';
    const { date } = getColombiaDateParts();

    const responsibles = await database.query(`
        SELECT r.*, s.name AS site_name
        FROM operational_site_responsibles r
        JOIN operational_sites s ON s.id = r.site_id
        WHERE r.active = TRUE AND s.active = TRUE
    ...
    const session = sessionManager.getSession(gpswoxSessionName);
    const sender = session && session.socket && session.state === config.SESSION_STATES.READY
        ? async (phone, message) => { await session.socket.sendMessage(toJid(phone), { text: message }); }
        : async (phone, message) => sessionManager.sendMessageWithRotation(phone, message);
    ...
    for (const responsible of responsibles.rows) {
        if (seenSites.has(responsible.site_id)) continue;

        const docs = await database.query(`
            SELECT plate, site_name, document_type, expiry_date,
                   CASE WHEN expiry_date IS NULL THEN NULL ELSE to_char(expiry_date, 'DD/MM/YYYY') END AS expiry_date_co,
                   CASE WHEN expiry_date IS NULL THEN NULL ELSE (expiry_date - CURRENT_DATE)::int END AS days_remaining
            FROM operational_document_expirations
            WHERE active = TRUE AND site_name ILIKE $1
              AND (expiry_date IS NULL OR expiry_date <= CURRENT_DATE + INTERVAL '30 days')
            ORDER BY expiry_date ASC NULLS LAST, plate
        `, [responsible.site_name]);

        if (docs.rows.length === 0) continue;

        seenSites.add(responsible.site_id);
        const message = buildDocumentFollowupMessage(responsible.site_name, docs.rows);

        await database.query(`
            INSERT INTO operational_document_followups
                (followup_date, site_id, responsible_id, phone_number, message, send_type, doc_count, sent_at)
            VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())
        `, [date, responsible.site_id, responsible.id, responsible.phone_number, message, sendType, docs.rows.length]);

        await sender(responsible.phone_number, message);
        sent.push({ phoneNumber: responsible.phone_number, siteName: responsible.site_name, docCount: docs.rows.length });
    }

    return sent;
}

async function getDocumentFollowupReport(filters = {}) {
    const params = [];
    const where = [];
    ...
    const baseQuery = `
        FROM operational_document_followups f
        JOIN operational_sites s ON s.id = f.site_id
    ...
    const selectSql = `
        SELECT f.id, f.followup_date, f.phone_number, f.send_type, f.sent_at, f.doc_count,
    ...
    if (filters.paginate) {
        ...
        const result = await database.query(`
            ${selectSql}
            ORDER BY f.followup_date DESC, s.name
            LIMIT $${queryParams.length - 1} OFFSET $${queryParams.length}
        `, queryParams);
        const total = countResult.rows[0]?.total || 0;
        return { rows: result.rows, pagination: buildPagination(total, page, limit) };
    ...
    const result = await database.query(`${selectSql} ORDER BY f.followup_date DESC, s.name LIMIT 500`, params);
    return result.rows;
}

async function deleteDocumentFollowup(id) {
    const result = await database.query(`DELETE FROM operational_document_followups WHERE id = $1 RETURNING *`, [id]);
    return result.rows[0] || null;
}

const FOLLOWUP_EXPIRY_MINUTES = 5;

async function expireStaleFollowups() {
    const result = await database.query(`
        UPDATE operational_followups
        SET status = 'expired', completed_at = NOW()
        WHERE status = 'sent' AND completed_at IS NULL
          AND sent_at < NOW() - INTERVAL '${FOLLOWUP_EXPIRY_MINUTES} minutes'
        RETURNING id, phone_number
    `);

    for (const row of result.rows) {
        const key = toJid(row.phone_number);
        const conversation = activeConversations.get(key);
        if (conversation && conversation.followupId === row.id) {
...
function startDailyScheduler(sessionManager) {
    ...
    schedulerInterval = setInterval(async () => {
        try {
            ...
            if (expired.length > 0) {
                console.log(`Seguimientos expirados por inactividad (${FOLLOWUP_EXPIRY_MINUTES} min): ${expired.length}`);
            }
        } catch (error) {
            console.error(`Error expirando seguimientos: ${error.message}`);
        }

        const parts = getColombiaDateParts();
        if (!isMondayToSaturday(parts.weekday)) return;

        const opRunKey = `${parts.date}-${NON_OPERATIONAL_PROMPT_HOUR}`;
        if (parts.hour === NON_OPERATIONAL_PROMPT_HOUR && parts.minute <= 10 && lastSchedulerRunKey !== opRunKey) {
            lastSchedulerRunKey = opRunKey;
            try {
        ...
        const docRunKey = `${parts.date}-${DOCUMENT_PROMPT_HOUR}`;
        if (parts.hour === DOCUMENT_PROMPT_HOUR && parts.minute <= 10 && lastSchedulerDocRunKey !== docRunKey) {
            lastSchedulerDocRunKey = docRunKey;
            try {
                const sent = await sendDocumentFollowups(sessionManager, { sendType: 'automatico' });
                console.log(`Seguimiento documentos enviado a ${sent.length} responsables`);
            } catch (error) {
                console.error(`Error enviando seguimiento documentos: ${error.message}`);
            }
        }
    }, 60 * 1000);

    console.log('Scheduler de operatividad activo: lunes a sabado 8:00 a.m. (operatividad) y 9:00 a.m. (documentos)');
}

async function cancelOpenFollowup(conversation) {
    if (!conversation || !conversation.phoneNumber) return;
    // Cierra TODOS los seguimientos abiertos del telefono, no solo el cargado en
    // memoria: un followup de un dia anterior sin responder queda con status='sent'
    // indefinidamente y "resucita" en el siguiente mensaje libre del usuario.
    await database.query(`
        UPDATE operational_followups
        SET status = 'cancelled', completed_at = NOW()
        WHERE phone_number = $1 AND status = 'sent' AND completed_at IS NULL
    `, [normalizeWhatsAppPhone(conversation.phoneNumber)]);
}
...
module.exports = {
    ...
    sendDocumentFollowups,
    getDocumentFollowupReport,
